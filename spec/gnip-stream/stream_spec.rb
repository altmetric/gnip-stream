...
describe GnipStream::Stream do
  ...
  describe '#on_message' do
    ...
    it 'accepts a block that will be called every time a message is received' do
      stream.on_message(&on_message_block)
      expect(stream.instance_variable_get(:@on_message)).to eq(on_message_block)
    end
  end
  ...
  describe '#on_connection_close' do
    ...
    it 'accepts a block that will be called every time a message is received' do
      stream.on_connection_close(&on_connection_close_block)
      expect(stream.instance_variable_get(:@on_connection_close)).to eq(on_connection_close_block)
    end
  end
  ...
  describe '#on_error' do
    ...
    it 'accepts a block that will be called every time a message is received' do
      stream.on_error(&on_error_block)
      expect(stream.instance_variable_get(:@on_error)).to eq(on_error_block)
    end
  end
  ...
  describe '#process_chunk' do
    ...
    it 'passes the chunk of data off to the processor object for processing' do
      ...
      stream.process_chunk('hello world')
    end
